...
from front.controls.date_picker_container import DatePickerContainer
from front.controls.time_picker_container import TimePickerContainer
...
class DateTimeFlexibleContainer(Container):
    ...
    @property
    def value(self):
        date = self._datepicker.value if self._datepicker else None
        time = self._timepicker.value if self._timepicker else None
        return ' '.join([date.strftime('%m/%d/%Y'), time.strftime('%H:%M')])

    @value.setter
    def value(self, value):
        if self._datepicker:
            self._datepicker.value = ''
        if self._timepicker:
            self._timepicker.value = ''
